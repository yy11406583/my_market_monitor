...
def send_msg(text):
    if not text or not TELEGRAM_TOKEN or not CHAT_ID: return
...
def get_stock_data(ticker_symbol):
    try:
        ...
        if not actions.empty and 'Dividends' in actions:
            ...
            divs = actions[actions.index > one_year_ago]['Dividends'].sum()
            dy = (divs / price * 100) if price > 0 else 0
        return dy, price
    except: return 0, 0
...
def run_main():
    ...
    if now_hour == 8:
        ...
        if maritime_hits:
            send_msg(f"🚨 <b>突發訊息 ({now_hour}:00)</b>\n\n{maritime_hits}")
        if 8 < now_hour < 24 and finance_hits:
            send_msg(f"🔔 <b>財經動態 ({now_hour}:00)</b>\n\n{finance_hits}")
