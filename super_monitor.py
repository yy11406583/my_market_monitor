...
# 雜訊排除名單
EXCLUDE_KEYWORDS = ["2房", "3房", "沽出", "易手", "地產", "公屋", "成交", "零議價", "房委會", "美容", "食店", "雞蛋仔", "NBA", "足球", "螺絲粉"]
...
def fetch_news_engine(history, mode="MARITIME"):
    ...
    try:
        ...
        for item in soup.find_all('item'):
            ...
            if mode == "MARITIME":
                # 關鍵邏輯：標題必須有「動作」才發放
                if any(act in t for act in ACTION_KEYWORDS): valid = True
            elif mode == "POLITICS":
...
def run_monitor():
    ...
    if now.hour == 8 and now.minute < 30:
        ...
        if urgent:
            send_telegram_msg(f"🔔 <b>即時情報 ({now.strftime('%H:%M')})</b>\n\n" + "\n\n".join(urgent))
