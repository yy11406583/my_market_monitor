...
# 關鍵字組合
MARITIME_KEYWORDS = ["水警", "走私", "快艇", "非法入境", "墮海", "警方", "警察", "反爆竊", "失蹤", "淋紅油"]
FINANCE_KEYWORDS = ["派息", "業績", "回購", "盈喜", "盈警", "股息", "減息", "加息", "聯儲局"]
POLITICS_KEYWORDS = ["特朗普", "川普", "伊朗", "戰爭", "核協議", "無人機", "導彈", "美軍", "中東局勢"]

EXCLUDE_KEYWORDS = ["台東", "台人", "甘肅", "川崎", "螺絲粉", "食安", "台灣好報"]
HK_DISTRICTS = ["香港", "港聞", "新界", "九龍", "港島", "屯門", "元朗", "機場", "旺角", "深水埗", "西貢", "觀塘", "灣仔", "中環", "南區", "沙田", "大角咀", "梅窩"]
...
def get_kdj_signal(ticker, interval):
    try:
        df = yf.Ticker(ticker).history(period="2y", interval=interval)
        val = calculate_kdj(df)
        return val, val < 20
    except: return 50.0, False
...
def is_duplicate(title, history):
    clean_t = re.sub(r'\(附圖\)|- 港聞|- 01|商报', '', title).strip()[:12]
    return any(clean_t in h for h in history)

def fetch_news(keywords, history, query, limit=8):
    ...
    try:
        ...
        for item in soup.find_all('item'):
            ...
            if any(k in t for k in keywords):
                if not is_duplicate(t, history + raw_titles):
                    found.append(f"{t.split(' - ')[0].strip()} ({src})")
                    raw_titles.append(t)
...
def run_main():
    ...
    status = {"last_politics_date": str(now.date())}
    if os.path.exists(STATUS_FILE):
        with open(STATUS_FILE, "r") as f: status = json.load(f)

    # 檢查 14 日期限
    last_p_date = datetime.strptime(status["last_politics_date"], "%Y-%m-%d").date()
    is_politics_active = (now.date() - last_p_date).days <= 14

    # 抓取新聞
    m_list, m_raw = fetch_news(MARITIME_KEYWORDS, history, "水警 OR 走私 OR 警方 OR 元朗")
    
    p_list, p_raw = [], []
    if is_politics_active:
        p_list, p_raw = fetch_news(POLITICS_KEYWORDS, history, "特朗普 OR 伊朗戰爭 OR 中東局勢", limit=5)
    ...
    if now.hour == 8 and now.minute < 30:
        ...
        for s, name in WATCHLIST.items():
            ...
            wk_v, wk_l = get_kdj_signal(s, "1wk")
            mo_v, mo_l = get_kdj_signal(s, "1mo")
            wk_s = f"🔥<b>{wk_v:.1f}</b>" if wk_l else f"{wk_v:.1f}"
            mo_s = f"💎<b>{mo_v:.1f}</b>" if mo_l else f"{mo_v:.1f}"
            rep.append(f"• {name}: <b>{price:.2f}</b> | 週:{wk_s} 月:{mo_s}")
        
        f_list, f_raw = fetch_news(FINANCE_KEYWORDS, [], "港股 派息 業績 盈喜")
